class PrintToClient
  ...
  def connect_machines(iserials)
    ...
    p [:print_to_client, :connect_machines]
    update_iserial_map
  ...
  def truncate_log
    # File.truncate("home/pi/PrintToPeer/logs/ptp_client.log", 0)
  end
...
private
    def connect_machine(port_name, baud, protocol)
      ...
      p [:spawn_burijji]
      # Process.spawn("~/PrintToPeer/bin/burijji -p /dev/#{port_name} -b #{baud} -s #{socket_location} -r '#{protocol}'")
      Process.spawn("~/PrintToPeer/bin/burijji-py.py -p /dev/#{port_name} -b #{baud} -s #{socket_location} -r '#{protocol}'")
